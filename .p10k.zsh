# Generated by Powerlevel10k configuration wizard on 2026-08-14 at 01:26 CEST.
...
() {
  ...
  typeset -g POWERLEVEL9K_LEFT_PROMPT_ELEMENTS=(
    ...
    my_dir                  # current directory (2-level custom)
    vcs                     # git status
  ...
  # Disable the non-writable directory warning icon.
  ...
  # Force virtualenv to display just ".venv".
  typeset -g POWERLEVEL9K_VIRTUALENV_CONTENT_EXPANSION='.venv'
  # Hide the Python icon so only ".venv" appears.
  ...
  # Custom dir segment: show at most 2 dirs after ~/, with only cwd highlighted.
  function prompt_my_dir() {
    local cwd=$PWD home=$HOME
    local fg_default=$POWERLEVEL9K_DIR_FOREGROUND
    local fg_shortened=$POWERLEVEL9K_DIR_SHORTENED_FOREGROUND
    local fg_anchor=$POWERLEVEL9K_DIR_ANCHOR_FOREGROUND
    local bold='' && [[ $POWERLEVEL9K_DIR_ANCHOR_BOLD == true ]] && bold='%B'

    [[ $cwd == '/' ]] && p10k segment -b '' -f "$fg_default" -t "%F{$fg_anchor}${bold}/%b%f" && return

    local prefix='' tail=()
    if [[ $cwd == $home ]]; then
      p10k segment -b '' -f "$fg_default" -t "%F{$fg_anchor}${bold}~%b%f"
      return
    elif [[ $cwd == $home/* ]]; then
      prefix='~'
      tail=(${(s:/:)${cwd#$home/}})
    ...
    p10k segment -b '' -f "$fg_default" -t "$content"
  }

  instant_prompt_my_dir() { prompt_my_dir; }
